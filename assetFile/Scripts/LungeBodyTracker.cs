...
public class LungeBodyTracker : MonoBehaviour
{
    ...
    private void SetMarkPos(GameObject effectPrefab, JointId jointId, Frame frame)
    {
        ...
        // 초기 위치 기준으로 Pelvis 관절의 Y축 위치가 5 이상 내려갔다가 초기 위치로 돌아오면 count 변수 증가
        if (jointId == JointId.Pelvis && initialPositionsSet)
        {
            if (pos.y < initialPelvisPosition.y - 5)
            {
                noseBelowThreshold = true;
            }
            else if (noseBelowThreshold && pos.y >= initialPelvisPosition.y - 5)
            {
                count++;
                noseBelowThreshold = false;
